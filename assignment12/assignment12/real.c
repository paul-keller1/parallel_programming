                  ...
                  double a[4], int k)
{
  ...
  #pragma omp parallel for private(i3, i2, i1) collapse(2) 
  for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
      double u1[M], u2[M];
      for (i1 = 0; i1 < n1; i1++) {
                  ...
                  void *os, int m1j, int m2j, int m3j, int k)
{
  ...
  #pragma omp parallel for private(j3, j2, j1, i3, i2, i1, x2, y2) collapse(2) 
  for (j3 = 1; j3 < m3j-1; j3++) {
    for (j2 = 1; j2 < m2j-1; j2++) {
      double x1[M], y1[M];  // Declare per-thread local arrays inside the parallel region
                   ...
                   void *ou, int n1, int n2, int n3, int k)
{
  ...
  double z1[M], z2[M], z3[M];  // only used in first big loop, will declare in parallel region there

  if (timeron) timer_start(T_interp);
  if (n1 != 3 && n2 != 3 && n3 != 3) {
    #pragma omp parallel for private(i3, i2, i1, z1, z2, z3) collapse(2) 
    for (i3 = 0; i3 < mm3-1; i3++) {
      for (i2 = 0; i2 < mm2-1; i2++) {
        double z1[M], z2[M], z3[M];  // declare here to ensure thread-private
