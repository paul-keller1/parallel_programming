...
void *mandelbrot_thread(void *arg) {
    ...
    for (int i = data->start; i < data->end; i++) {
        for (int j = 0; j < Y; j++) {
            ...
            uint8_t intensity = (uint8_t)(k * 255 / MAX_ITER);
            data->image[j][i * 3 + 0] = (65 * intensity) / 255;
            data->image[j][i * 3 + 1] = (14 * intensity) / 255;
            data->image[j][i * 3 + 2] = (102 * intensity) / 255;
...
void calc_mandelbrot(uint8_t image[Y][X*3], int N) {
    pthread_t threads[N];
...
int main(int argc, char *argv[]) {
    ...
    uint8_t image[Y][X*3]; // 3 channels for RGB
    ...
    stbi_write_png("mandelbrot_color.png", X, Y, channel_nr, image, stride_bytes);
    return EXIT_SUCCESS;
}